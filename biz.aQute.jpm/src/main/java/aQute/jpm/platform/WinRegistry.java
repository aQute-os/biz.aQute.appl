...
public class WinRegistry implements WindowsRegistryAccessProvider {
	...
	private static Pattern						REGISTRY_REFERENCE_REGEX	= Pattern
		.compile("\\$\\(Registry:([A-Z_]+)\\\\(.*)@(.*)\\)");
	...
	private static long deleteValue(Preferences root, long hkey, String key, String value)
		...
		long rc = ((Long) regDeleteValue.invoke(root, new Object[] {
	...
	private static Map<String, String> readStringValues(Preferences root, int hkey, String key)
		...
		if (onJava11) {
			...
			long[] info = (long[]) regQueryInfoKey.invoke(root, new Object[] {
				new Long(handles[0])
			});
	...
	private static long[] createKey(Preferences root, long hkey, String key)
		throws IllegalArgumentException, IllegalAccessException, InvocationTargetException {
		return (long[]) regCreateKeyEx.invoke(root, new Object[] {
			new Long(hkey), toCstr(key)
		});
	}
	...
	@Override
	public String getValue(RegistryHKey registryHKey, String key, String valueName)
		throws WindowsRegistryAccessException {
		try {
			...
			throw new WindowsRegistryAccessException("NPANDAY-117-000: Error while retrieving a windows registry value",
				e);
		} catch (IllegalAccessException e) {
			throw new WindowsRegistryAccessException("NPANDAY-117-001: Error while retrieving a windows registry value",
				e);
		}
	}
}
