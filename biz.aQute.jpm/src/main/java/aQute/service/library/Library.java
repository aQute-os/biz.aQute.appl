...
public interface Library {
	...
	String	SIMPLE_NAME			= "[-\\p{L}0-9_.]+";
	Pattern	SIMPLE_NAME_PATTERN	= Pattern.compile(SIMPLE_NAME);

	Pattern	COORDINATES_P		= Pattern.compile(
		"\\s*([a-z0-9-_.]+)(?:\\s*:\\s*([a-z0-9-_.]+)(?:\\s*:\\s*([a-z0-9-_.]*))?)?(?:\\s*@\\s*([<>=]?[a-z0-9-_.,\\[\\]\\(\\)]+))?\\s*",
		Pattern.CASE_INSENSITIVE);
	Pattern	SHA_P				= Pattern.compile("\\s*([a-f0-9]{40,40})\\s*", Pattern.CASE_INSENSITIVE);

	Pattern	MASTER_QUALIFIER	= Pattern.compile("|RELEASE|FINAL|GA|GM|GOLD", Pattern.CASE_INSENSITIVE);
	 ...
	 *
	 * @param coordinates standard coordinates
