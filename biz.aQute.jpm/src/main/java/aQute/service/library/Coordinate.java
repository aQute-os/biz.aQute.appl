...
/**
 * A core concept in the Library is the coordinate, it designates a range of
 * revisions or programs. The syntax of the Coordinate is:
 *
 * <pre>
...
public class Coordinate {
	...
	public boolean isValid(String coordinate) {
		return COORDINATE_P.matcher(coordinate.replaceAll("__", ":"))
			.matches();
	}
	...
	public Coordinate(Matcher matcher) {
		if (!matcher.matches())
			throw new IllegalArgumentException(
				matcher.group() + " does not match coordinate pattern: " + matcher.pattern()
					.pattern());
		...
		if (artifactId == null && classifier == null) {
			artifactId = groupId;
			if (Library.SHA_P.matcher(groupId)
				.matches()) {
